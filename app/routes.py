...
def book_list():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            rental_from_form = Rental.query.filter_by(status=form.data['rental']).first()
            new_book = Book(tittle=form.data["tittle"], author_id=author_from_form.id, rental_id=rental_from_form.id)
